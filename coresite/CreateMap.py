...
def create_map_frame(num_date, num_time, opacity, color):
    ...
    if num_date is not None and num_time is not None:
        ...
        for device in magog_sensor.objects.values_list('device_id'):
            lat = magog_sensor.objects.filter(device_id__iexact='{device_id}'.format(device_id=device[0])).values(
                "web_lat")[
                0]["web_lat"]
            lon = magog_sensor.objects.filter(device_id__iexact='{device_id}'.format(device_id=device[0])).values(
                "web_lon")[
                0]["web_lon"]
            temp = magog_uhi.objects.filter(device_id__iexact='{device_id}'.format(device_id=device[0]),
                                            ...
                                            time__hour__lte=int(num_time)).values("temperature")[0]["temperature"]
    ...
    # If we don't have proper data to interpolate, return an empty map.
    else:
        folium.LayerControl().add_to(m)
        f = m._repr_html_()
